...
// Inluye funciones de conversión BCD-Binario: bin2bcd y bcd2bin
// y de lectura/escritura a través de I2C de registros individuales del PCF:
// PCF8583_read_byte y PCF8583_write_byte.
//
//Las funciones principales que define son:
//PCF8583_set_date_time: para fijar fecha y hora inicial 
//PCF8583_read_date_time: para leer fecha y hora actual
//PCF8583_set_daily_alarm: borra el flag de alarma y programa la hora de la siguiente
//                 alarma diaria (sólo importa la hora de alarma, el mes y día se ignoran)
//                 para que avise por interrupción (registro de control-estado=0x02 y 
//                 registro de control_de_alarma=0x90)
//
//Los datos de fecha y hora los pasa el usuario mediante una estructura
//del tipo: date_time_t
...
//Estructura para almacenar la hora de alarma diaria
typedef struct{
 //  int8 seconds;    // 0 to 59
   int8 minutes;    // 0 to 59
   int8 hours;      // 0 to 23  (24-hour time)
...
//---------------------------------------------------------------
// Funcion que define la alarma como 'alarma diaria', 
// generando interrupción,
// y pone la hora de dicha alarma.
//NO incluye la activación de la alarma, que deberá
//ordenarse activando el bit 2 del registro de control/estado

void PCF8583_set_daily_alarm(alarm_time *at){
 //  int8 bcd_sec;
   int8 bcd_min;
   int8 bcd_hrs;
  
 //  bcd_sec = bin2bcd(at->seconds);
   bcd_min = bin2bcd(at->minutes);
   bcd_hrs = bin2bcd(at->hours);

  //PCF8583_write_byte(PCF8583_CTRL_STATUS_REG,0x04);
  

   disable_interrupts(GLOBAL);
   i2c_start();
   i2c_write(PCF8583_WRITE_ADDRESS);
   i2c_write(PCF8583_ALARM_CONTROL_REG);
   i2c_write(0x90);              // alarma diaria, por interrupción
   i2c_write(0x00);               // Set 100's reg = 0
   i2c_write(0x00);              //pone a 0 los segundos
  // i2c_write(bcd_sec);
   i2c_write(bcd_min);
   i2c_write(bcd_hrs);
   i2c_stop();
